...
const gchar * helia_mpegts_enum_name ( GType instance_type, gint val )
{
...
void helia_mpegts_clear_scan ()
{
	...
	g_debug ( "helia_mpegts_clear_scan" );
}


static void helia_mpegts_pat ( GstMpegtsSection *section )
{
...
static void helia_mpegts_pmt ( GstMpegtsSection *section )
{
	...
	for ( i = 0; i < len; i++ )
	{
		...
		g_debug ( "       pid: %d (0x%04x), stream_type:0x%02x (%s)", stream->pid, stream->pid, stream->stream_type,
			helia_mpegts_enum_name (GST_TYPE_MPEGTS_STREAM_TYPE, stream->stream_type) );

		const gchar *name_t = helia_mpegts_enum_name ( GST_TYPE_MPEGTS_STREAM_TYPE, stream->stream_type );
...
static void helia_mpegts_sdt ( GstMpegtsSection *section )
{
			...
			if ( desc->tag == GST_MTS_DESC_DVB_SERVICE )
			{
				if ( gst_mpegts_descriptor_parse_dvb_service ( desc, &service_type, &service_name, &provider_name ) )
				{
					...
					g_debug ( "   Service Descriptor, type:0x%02x (%s)",
						service_type, helia_mpegts_enum_name (GST_TYPE_MPEGTS_DVB_SERVICE_TYPE, service_type) );
...
static void helia_mpegts_vct ( GstMpegtsSection *section )
{
...
void helia_mpegts_parse_section ( GstMessage *message )
{
	...
	// g_debug ( "helia_mpegts_parse_section" );
}
